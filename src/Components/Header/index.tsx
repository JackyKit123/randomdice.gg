...
function ProfilePopup(): JSX.Element {
  ...
  return (
    <>
      <h3>Profile</h3>
      <span>
        You can update your Displayed Name and link other account to your
        profile here. Keep in mind that the accounts should be associated with
        the same email address in this account.
      </span>
      ...
      <div className='oauth'>
        ...
        {!accountLinked.includes('patreon') ? (
          <button
            ...
            className='patreon'
            type='button'
            onClick={(): Promise<void> => loginPatreon(dispatch, true)}
          >
            <FontAwesomeIcon icon={faPatreon} />
          </button>
        ) : null}
          ...
          <div className='container'>
            {auth && auth !== 'awaiting auth state' ? (
              <>
                ...
                <span className='user'>Welcome, {auth.displayName}!</span>
                <button
                  aria-label='edit profile'
                  type='button'
                  className='update'
                  onClick={(): void => openPopup(<ProfilePopup />)}
                >
