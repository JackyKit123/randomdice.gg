...
export default function BattlefieldGuide(): JSX.Element {
  ...
  useEffect(() => {
    if (
      ...
      setBattlefieldInfo(battlefield);
    }
  }, [wiki]);
  ...
  return (
    ...
    >
      <p>
        Since patch 5.3.0, battlefield skins now give a unique buff in addition
        to being a cosmetic. The buff given by each individual battlefield skin
        can be viewed below.
      </p>
      <p>
        The battlefield buff effect can be leveled up using gold. The maximum
        level for the battlefield skin is your pvp rank, meaning that level 20
        is the maximum level for all battlefield skins. Below is a table showing
        the cost to upgrade the battlefield skins. It takes a total of{' '}
        {Array.from(coinsPerLevel.values()).reduce((acc, cur) => acc + cur)}{' '}
        gold to upgrade a battlefield skin to max level.
      </p>
      <p>
        In co-op mode, the battlefield from the player with higher pvp rank will
        be used. If both players are the same pvp rank, the battlefield used is
        randomly picked. The only exception is that the default light mode
        battlefield skin is never chosen despite one player having higher pvp
        rank.
      </p>
        ...
        {battlefieldInfo?.map(battlefield =>
          battlefield.img === 'ad' ? (
            ...
            <Fragment key={battlefield.name}>
              ...
              <div className='battlefield' id={battlefield.name}>
                <figure>
                  <img src={battlefield.img} alt={battlefield.name} />
                </figure>
                ...
                <div>
                  ...
                  {Math.round(
                    (battlefield.buffValue +
                      battlefield.buffCupValue * battleFieldLevel) *
                      100
                  ) / 100}
                ...
                <label htmlFor='battlefield-level'>
                  ...
                  <select
                    onChange={(evt): void => {
                      setBattlefieldLevel(Number(evt.target.value));
                ...
                <div>Obtained from: {battlefield.source}</div>
                {ReactHtmlParser(sanitize(battlefield.desc))}
              </div>
            </Fragment>
          )
        )}
