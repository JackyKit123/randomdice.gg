...
export default function EnergyCalculator(): JSX.Element {
    ...
    switch (mode) {
      case 'raw':
        return {
          dmg: roundTo3Sf(dmg),
          dps: roundTo3Sf(
            (dmg * 0.95 + dmg * 0.05 * (filter.crit / 100)) / data.energy.spd
          ),
        };
      case 'crit': {
        const critMultiplier =
          (5 +
            ((filter.critical.class - 3) * data.crit.cupEff1 + data.crit.eff1) *
              filter.critical.pip +
      ...
      case 'light': {
        const spdBuff =
          1 -
          ((data.light.eff1 + data.light.cupEff1 * (filter.light.class - 3)) *
            filter.light.pip +
            data.light.pupEff1 * (filter.light.level - 1)) /
            100;
        const atkSpd =
          spdBuff * data.energy.spd <= 0.01 ? 0.01 : spdBuff * data.energy.spd;
        return {
          dmg: roundTo3Sf(dmg),
          dps: roundTo3Sf(
            (dmg * 0.95 + dmg * 0.05 * (filter.crit / 100)) / atkSpd
          ),
        };
      }
      case 'lunar': {
        ...
        return {
          ...
          dps: roundTo3Sf(
            (buffedDmg * (1 - critMultiplier) +
              (buffedDmg * critMultiplier * filter.crit) / 100) /
              atkSpd
          ),
        };
      }
  ...
  return (
    ...
    >
      <p>
        This is a calculator for calculating the Energy Dice damage. You can see
        the dps of energy raw damage and the dps of energy when it is buffed by
        Light, Critical or Lunar Dice.
      </p>
      <p>
        Do Remember that damage and dps shown is per pip. You will have to
        multiply the total pip count on your board to get the final dps.
      </p>
          ...
          >
            <label htmlFor='class'>
              ...
              <select
                name='class'
                onChange={(evt: React.ChangeEvent<HTMLSelectElement>): void => {
                  filter.energy.class = Number(evt.target.value);
                  setFilter({ ...filter });
                }}
              >
                <option>3</option>
            ...
            <label htmlFor='energy level'>
              ...
              <select
                name='energy level'
                onChange={(evt: React.ChangeEvent<HTMLSelectElement>): void => {
                  filter.energy.level = Number(evt.target.value);
                  setFilter({ ...filter });
                }}
              >
                <option>1</option>
          ...
          <label htmlFor='result'>
            ...
            <input
              ...
              value={
                invalidInput
                  ? 'Check Input'
                  : Math.round((dpsPerSpCount('raw').dmg * filter.crit) / 100)
              }
              disabled
            />
          </label>
          ...
          <label htmlFor='result'>
            ...
            <input
              ...
              value={
                invalidInput
                  ? 'Check Input'
                  : Math.round((dpsPerSpCount('lunar').dmg * filter.crit) / 100)
              }
              disabled
            />
          </label>
        ...
        <div className='dps'>
          ...
          <label htmlFor='result'>
            ...
            <input
              type='textbox'
              className={invalidInput ? 'invalid' : ''}
              value={invalidInput ? 'Check Input' : dpsPerSpCount('raw').dps}
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              type='textbox'
              className={invalidInput ? 'invalid' : ''}
              value={invalidInput ? 'Check Input' : dpsPerSpCount('crit').dps}
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              type='textbox'
              className={invalidInput ? 'invalid' : ''}
              value={invalidInput ? 'Check Input' : dpsPerSpCount('light').dps}
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              type='textbox'
              className={invalidInput ? 'invalid' : ''}
              value={invalidInput ? 'Check Input' : dpsPerSpCount('lunar').dps}
              disabled
            />
          </label>
        </div>
      ...
      <div className='chart-container'>
        <VictoryChart
          ...
          maxDomain={{
            ...
            y:
              Math.max(
                dpsPerSpCount('crit', filter.energy.sp + 100).dps,
                dpsPerSpCount('lunar', filter.energy.sp + 100).dps
              ) + 10,
          }}
        ...
        >
            ...
            tickFormat={(t): string => {
              switch (true) {
                ...
                case t > 999999999999999999:
                  return `${Math.round(t / 100000000000000000) / 10}T`;
          ...
          <VictoryLegend
            ...
            gutter={20}
            colorScale={['#197cf0', '#ff0000', '#ffff00', '#111111']}
            data={[
              { name: 'Lunar Buffed' },
              { name: 'Crit Buffed' },
              { name: 'Light Buffed' },
          ...
          <VictoryLine
            ...
            y={(d: { x: number }): number => dpsPerSpCount('lunar', d.x).dps}
          />
          <VictoryLine
            name='Light Buffed'
            samples={100}
            ...
            y={(d: { x: number }): number => dpsPerSpCount('light', d.x).dps}
          />
          <VictoryLine
            name='Crit Buffed'
            samples={100}
            ...
            y={(d: { x: number }): number => dpsPerSpCount('crit', d.x).dps}
          />
          <VictoryLine
            name='No Buff'
            samples={100}
            ...
            y={(d: { x: number }): number => dpsPerSpCount('raw', d.x).dps}
          />
        </VictoryChart>
      </div>
    </PageWrapper>
