...
export default function ComboCalculator(): JSX.Element {
    ...
    switch (mode) {
      case 'raw':
        return {
          dmg: roundTo3Sf(dmg),
          dps: roundTo3Sf(
            (dmg * 0.95 + dmg * 0.05 * (filter.crit / 100)) / data.combo.spd
          ),
        };
      case 'crit': {
        return {
          ...
          dps: roundTo3Sf(
            (dmg * (1 - criticalCritMultiplier) +
              (dmg * criticalCritMultiplier * filter.crit) / 100) /
              data.combo.spd
          ),
        };
      }
        ...
        return {
          ...
          dps: roundTo3Sf(
            (lunarBuffedDmg * (1 - lunarCritMultiplier) +
              (lunarBuffedDmg * lunarCritMultiplier * filter.crit) / 100) /
              atkSpd
          ),
        };
      }
      case 'lunar+crit': {
        ...
        return {
          ...
          dps: roundTo3Sf(
            (lunarBuffedDmg *
              (1 - Math.max(lunarCritMultiplier, criticalCritMultiplier)) +
              (lunarBuffedDmg *
                Math.max(lunarCritMultiplier, criticalCritMultiplier) *
                filter.crit) /
                100) /
              atkSpd
          ),
  ...
  const plotGraphCallback = {
    ...
    raw: useCallback(
      ...
      [filter.combo.class, filter.combo.level, filter.combo.count, filter.crit]
    ),
  };
  ...
  return (
    ...
    >
      <p>
        This is a calculator for calculating the Combo Dice damage. You can see
        the dps of combo raw damage and the dps of combo when it is buffed by
        Critical and Lunar Dice.
      </p>
      <p>
          ...
          >
            <label htmlFor='class'>
              ...
              <select
                name='class'
                onChange={(evt: React.ChangeEvent<HTMLSelectElement>): void => {
                  filter.combo.class = Number(evt.target.value);
                  setFilter({ ...filter });
                }}
              >
                <option>7</option>
            ...
            <label htmlFor='combo level'>
              ...
              <select
                name='combo level'
                onChange={(evt: React.ChangeEvent<HTMLSelectElement>): void => {
                  filter.combo.level = Number(evt.target.value);
                  setFilter({ ...filter });
                }}
              >
                <option>1</option>
        ...
        <div className='dmg'>
          ...
          <label htmlFor='result'>
            ...
            <input
              ...
              value={invalidInput ? 'Check Input' : dpsPerComboCount('raw').dmg}
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              ...
              value={
                invalidInput
                  ? 'Check Input'
                  : Math.round(
                      (dpsPerComboCount('raw').dmg * filter.crit) / 100
                    )
              }
              disabled
            />
          ...
          <label htmlFor='result'>
            ...
            <input
              ...
              className={invalidInput ? 'invalid' : ''}
              value={
                invalidInput ? 'Check Input' : dpsPerComboCount('lunar').dmg
              }
              disabled
            />
          </label>
            ...
            <input
              ...
              value={
                invalidInput
                  ? 'Check Input'
                  : Math.round(
                      (dpsPerComboCount('lunar').dmg * filter.crit) / 100
                    )
              }
              disabled
            />
        ...
        <div className='dps'>
          ...
          <label htmlFor='result'>
            ...
            <input
              type='textbox'
              className={invalidInput ? 'invalid' : ''}
              value={invalidInput ? 'Check Input' : dpsPerComboCount('raw').dps}
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              ...
              className={invalidInput ? 'invalid' : ''}
              value={
                invalidInput ? 'Check Input' : dpsPerComboCount('crit').dps
              }
              disabled
            />
          </label>
          <label htmlFor='result'>
            ...
            <input
              ...
              className={invalidInput ? 'invalid' : ''}
              value={
                invalidInput ? 'Check Input' : dpsPerComboCount('lunar').dps
              }
              disabled
            />
          </label>
      ...
      <div className='chart-container'>
        <VictoryChart
          ...
          maxDomain={{
            ...
            y: Math.max(
              dpsPerComboCount('lunar', filter.combo.count + 10).dps,
              dpsPerComboCount('crit', filter.combo.count + 10).dps
            ),
          }}
