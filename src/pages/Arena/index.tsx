...
export default function ArenaDraft(): JSX.Element {
  ...
  const calDiceRole = (diceId: number): number => {
    ...
    if (maxValue) {
      const diceRole = <T extends keyof DiceValue>(
        ...
        Math.round(
          (Math.max(targetScore - deckScore(deckType), 0) / targetScore) *
            10 *
            100
        ) / 100;
      switch (maxValue[0]) {
  ...
  return (
    <PageWrapper
      ...
      description='An Interactive Arena Drafter to assist you on building your arena deck for Random Dice! Put your options and compare the values, make your best deck for your 12wins game.'
    >
      <p>
        This is a Tool for assisting to draft an arena deck. To start drafting,
        input you 3 dice in the section below. Once all 3 dices are input into
        the picks section, you can compare the value and pick into one of them.
        Or if you are confident, you can skip comparing and use the bypass
        option below the table.
      </p>
      <p>
        Usually, highest dice role value will result in the best pick. If the
        highest dice role value are equal, the highest dice value between them
        will be the best pick.
      </p>
      <p>
        Take note that dice like typhoon or blizzard may be a good pick even if
        their dice role are low. And you should usually pick into typhoon and
        blizzard.
      </p>
      <p>
        Once you finished picking you deck, we will show a rough estimated win
        rate on the deck based on the deck score of your deck.
      </p>
      <p>
        Lastly, do be reminded that the legendary dice is not available for the
        first 2 slots in your deck, the same thing applies for this tool.
      </p>
      ...
      <section className='pick'>
        ...
        <div className='table-container'>
          <table className='pick'>
            <tbody>
                ...
                {[1, 2, 3].map((tdIndex, i) => (
                  <td key={`pick${tdIndex}`}>
                    <label htmlFor='option'>
                      <div>Option {i + 1}</div>
                      <select
                        data-value={pick[tdIndex] < 0 ? '?' : pick[tdIndex]}
                        value={pick[tdIndex]}
                        onChange={(evt): void => {
                          pick[tdIndex] = Number(evt.target.value);
                          setPick({ ...pick });
                        }}
                      >
                        ...
                        {dice
                          ...
                          .filter(die => {
                            ...
                            delete activePicks[tdIndex];
                            return ![
                              ...Object.values(deck),
                              ...Object.values(activePicks),
                            ].includes(die.id);
                          })
                          .map(die => (
                            <option
                ...
                {[1, 2, 3].map(tdIndex => (
                  <td
                    className={`dice-container ${
                      Object.values(pick).includes(-1) ? '' : 'active'
                    }`}
                    key={tdIndex}
                    // eslint-disable-next-line jsx-a11y/no-noninteractive-tabindex
                    tabIndex={Object.values(pick).includes(-1) ? -1 : 0}
                    onClick={(): void => pickDice(tdIndex)}
                    onKeyDown={(evt): void => {
              ...
              <tr>
                {[1, 2, 3].map(tdIndex => (
                  <td className='value' key={`diceRole${tdIndex}`}>
              ...
              <tr>
                {[1, 2, 3].map(tdIndex => (
                  <td className='value' key={`value${tdIndex}`}>
                    {Math.max(
                      ...(Object.values(
        ...
        <div className='manual'>
          I do not need to compare the value, put this dice into the deck
          directly:{' '}
          <form
            onSubmit={(evt): void => {
              evt.preventDefault();
          ...
          >
            <label htmlFor='manual-select'>
              <select
                ref={manualPickRef}
                data-value={manualPick < 0 ? '?' : manualPick}
              ...
              >
                ...
                {dice
                  ...
                  .map(die => (
                    <option value={die.id} key={`pick--${die.id}`}>
                      {die.name}
                    </option>
                  ))}
              </select>
        ...
        <button
          type='button'
          disabled={[...Object.values(deck), ...Object.values(pick)].every(
            val => val < 0
          )}
          onClick={(): void => {
            setPick(emptyPick);
      ...
      <section className='deck'>
        ...
        <label htmlFor='Assist-Dps'>
          ...
          <input type='textbox' value={deckScore('assist')} disabled />
        </label>
        <label htmlFor='Slow'>
      ...
      <section className={`winrate ${currentPick === 6 ? 'show' : ''}`}>
        <hr className='divisor' />
        ...
        <label htmlFor='legendary-reward'>
          ...
          <input
            ...
            value={
              Math.round(
                (winProb[4] * 0.1 + winProb[7] * 0.4 + winProb[11] * 1) * 10000
              ) / 10000 || 0
            }
            disabled
          />
        ...
        <label htmlFor='diamond-reward'>
          ...
          <input
            ...
            value={
              Math.round(
                (winProb[1] * 20 + winProb[6] * 80 + winProb[9]) * 100
              ) / 100 || 0
            }
            disabled
          />
        ...
        {Array(12)
          ...
          .map((_, i) => (
            <label htmlFor='win-chance' key={winProb[i] || i}>
              ...
              <input
                type='textbox'
                value={`${Math.round(winProb[i + 1] * 10000) / 100}%`}
                disabled
              />
            </label>
          ))}
