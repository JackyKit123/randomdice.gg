  ...
  return (
    <div className='deck-list'>
      ...
      {alternatives
        .map(alt => ({
          id: alt,
          alternatives: dice?.find(die => die.id === alt)?.alternatives,
        }))
        .map(die => (
          <div key={die.id}>
            <Dice die={die.id} />
              ...
              <>
                ...
                <div className='replacement'>
                  {die.alternatives.list?.map(altDice =>
                    altDice ? <Dice die={altDice} key={altDice} /> : null
                  )}
                </div>
              </>
            ) : (
  ...
  return (
    <PageWrapper
      isContentReady={Boolean(
        dice?.length && decks?.length && guide?.length && battlefield?.length
      )}
      error={firebaseError}
      retryFn={(): void => {
        fetchDecks(dispatch);
      ...
      title={`Deck List (${
        deckType === 'pvp' ? 'PvP' : deckType === 'co-op' ? 'Co-op' : 'Crew'
      })`}
    ...
    >
      <p>
        This is a interactive deck list for PVP, PVE and Crew decks. You can
        filter the legendary you have below. In this page{' '}
        <strong>{deckType} decks</strong> are shown, you can switch the deck
        type below. You can also specify a dice in Custom Search, which will
        show the decks with the dice you specified.
      </p>
      <p>
        We know that that not everyone have every legendary dices for every
        decks, so you can click on the button in alternatives column to show
        yourself some alternative options for some legendary dice.
      </p>
      <p>
        You can choose to have varying legendary class and crit% to see
        different ratings for certain decks at certain dice class or level.
      </p>
      <p>
        {' '}
      ...
      <p className='updated'>
        ...
        {new Date(
          JSON.parse(localStorage.getItem('last_updated') || '{}').decks
        ).toDateString()}
        .
      </p>
            ...
            {sortedDeck.length > 0 ? (
              sortedDeck.map(deckInfo => (
                <tr key={`deck-${deckInfo.id}`}>
                  <td>{deckInfo.type === '-' ? '-' : deckInfo.id}</td>
                  <td>
                    {deckInfo.type === '-'
                      ? '-'
                      : deckInfo.rating[profile] || deckInfo.rating.default}
                  </td>
                  <td>
                  ...
                  {deckType === 'crew' ? null : (
                    <td>
                      <Link
                        ...
                        to={`/wiki/battlefield#${
                          battlefield.find(b => b.id === deckInfo.battlefield)
                            ?.name
                        }`}
                      >
                        {
                    ...
                    <div className='button-container'>
                      {deckInfo.decks.map((deck, i) => (
                        <button
                          // eslint-disable-next-line react/no-array-index-key
                          key={i}
                          disabled={deckInfo.type === '-'}
                          aria-label='see alternatives'
                          type='button'
                          onClick={(): void =>
                            openPopup(<AlternativesList alternatives={deck} />)
                          }
                        >
                        ...
                        return (
                          <Link
                            ...
                            to={`/decks/guide/${guide?.find(
                              eachGuide =>
                                deckInfo.guide[i] === eachGuide.id ||
                                eachGuide.diceList.find(
                                  list =>
                                    list.every(dieId => deck.includes(dieId)) &&
                                    deckInfo.type === eachGuide.type
                                )
                            )?.name || ''}`}
                          >
