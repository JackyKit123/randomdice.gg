...
export default function critDataCollection(): JSX.Element {
  ...
  for (let i = 1; i <= 20; i += 1) {
    ...
    const min = critDataPerClass.length > 0 ? math.min(critDataPerClass) : 0;
    const max = critDataPerClass.length > 0 ? math.max(critDataPerClass) : 0;
    const avg = critDataPerClass.length > 0 ? math.mean(critDataPerClass) : 0;
    const median =
  ...
  return (
    <PageWrapper
      ...
      title='Crit% Data'
      className='crit-data'
      description='Data gathered from the Random Dice community for their respective crit% across different ranks! Compare your own crit% to the majority of the player base.'
    >
      <p>
        The following data is collected through the community. Please consider
        contributing by giving us your crit data in here. The data collected is
        anonymous.
      </p>
      {auth && auth !== 'awaiting auth state' ? (
        <>
          <p>
            Please enter your data in the following form. The data you entered
            will go live immediately. You do not need to enter your rank, it is
            calculated automatically based on your trophies.
          </p>
          <p>
            Note that your trophies count should be a positive integer while
            your crit damage should be between 111 - {maxCrit}. If you are class
            20 or above, your trophies count will be rounded down to 9,000
            trophies.
          </p>
          <form>
            <label htmlFor='my-trophies'>
              ...
              <input
                ...
                min={0}
                step={1}
                defaultValue={critData?.[auth.uid]?.trophies || myTrophies}
                onChange={(evt): void => {
                  evt.persist();
                  clearTimeout(typingTrophies);
                  setTypingTrophies(() =>
                    window.setTimeout(async (): Promise<void> => {
                      const trophies = Number(evt.target.value);
                      ...
                      if (trophies >= 0 && auth) {
                        await Promise.all([
                          ...
                          database
                            .ref(`/critData/${auth.uid}/crit`)
                            .set(myCrit),
                          database
                            .ref(`/critData/${auth.uid}/trophies`)
                            .set(trophies),
                        ]);
                        fetchCrit(dispatch);
                      }
                ...
                value={
                  Number(
                    Object.entries(trophiesForClass)
                      ...
                      .find(
                        // eslint-disable-next-line @typescript-eslint/no-unused-vars
                        ([_, trophies]) => trophies <= myTrophies
                      )?.[0]
                  ) || 1
                }
                disabled
            ...
            <label htmlFor='my-trophies'>
              ...
              <input
                type='number'
                className={myCrit < 111 || myCrit > maxCrit ? 'invalid' : ''}
                min={111}
                step={1}
                max={maxCrit}
                defaultValue={critData?.[auth.uid]?.crit || myCrit}
                onChange={(evt): void => {
                  evt.persist();
                  ...
                  setTypingCrit(() =>
                    window.setTimeout(async (): Promise<void> => {
                      ...
                      setMyCrit(crit);
                      if (crit >= 111 && crit < maxCrit && auth) {
                        await Promise.all([
                          database
                            .ref('/last_updated/critData')
                            .set(new Date().toISOString()),
                          database.ref(`/critData/${auth.uid}/crit`).set(crit),
                          database
                            .ref(`/critData/${auth.uid}/trophies`)
                            .set(myTrophies),
                        ]);
        ...
        <p>
          Please{' '}
          <button type='button' onClick={(): void => openPopup(<LoginPopup />)}>
            Login
          </button>{' '}
          to enter your data.
        </p>
      ...
      <div className='chart-container'>
        ...
        >
          ...
          <VictoryLine
            ...
            data={Object.entries(processedData).map(([rank, data]) => ({
              x: rank,
              y: data.avg,
            }))}
          />
      ...
      <div className='table-container'>
          ...
          <tbody>
            <tr>
              ...
              {Object.keys(trophiesForClass)
                .slice(0, 10)
                .map(classNum => (
                  <td key={`crit-table-class-${classNum}`}>{classNum}</td>
                ))}
            </tr>
            <tr>
              ...
              {Object.values(trophiesForClass)
                .slice(0, 10)
                .map(trophies => (
                  <td key={`crit-table-trophies-${trophies}`}>{trophies}</td>
                ))}
            </tr>
            <tr>
                ...
                .map(([rank, data]) => (
                  <td key={`crit-table-med-${rank}`}>
                    {Math.round(data.median)
                      ? Math.round(data.median * 100) / 100
                      : ''}
                  </td>
                ))}
            </tr>
        ...
        <table className='vertical'>
          ...
          <tbody>
            {Object.entries(processedData).map(([rank, data]) => (
              <tr key={`crit-table-row-${rank}`}>
                <td>{rank}</td>
                <td>{data.trophies}</td>
                <td key={`crit-table-min-${rank}`}>
                  {Math.round(data.min) ? Math.round(data.min * 100) / 100 : ''}
                </td>
                <td key={`crit-table-avg-${rank}`}>
                  {Math.round(data.avg) ? Math.round(data.avg * 100) / 100 : ''}
                </td>
                <td key={`crit-table-med-${rank}`}>
                  {Math.round(data.median)
                    ? Math.round(data.median * 100) / 100
                ...
                <td key={`crit-table-max-${rank}`}>
                  {Math.round(data.max) ? Math.round(data.max * 100) / 100 : ''}
                </td>
              </tr>
            ))}
          </tbody>
