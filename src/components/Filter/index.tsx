...
function DeckProfileFilter(): JSX.Element {
  ...
  return (
    <label htmlFor='profile'>
      ...
      <select
        value={profile}
        onChange={(evt): void => setProfile(evt.target.value as typeof profile)}
      >
  ...
  return (
    <label htmlFor='Custom Search'>
      ...
      <select
        name='Custom Search'
        value={customSearch}
        onChange={(evt): void => setCustomSearch(Number(evt.target.value))}
                ...
                setLegendaryOwned(
                  legendaryOwned.includes(die)
                    ? legendaryOwned.filter(
                        dieId => dieId !== Number(evt.target.value)
                      )
                    : [...legendaryOwned, Number(evt.target.value)]
                )
              }
            />
